        ...
        if ($request->initSession() && $request->isSafe() && $login = Login::isMember()) {
            if (Login::checkPermission($login, 'can_config') && Login::notDemoMode($login)) {
                try {
                    ...
                    foreach (array('user_forgot', 'user_register', 'welcome_email', 'member_only', 'demo_mode', 'show_title_logo', 'activate_user', 'new_members_active') as $key) {
                    ...
                    foreach (array('bg_color', 'color', 'warpper_bg_color') as $key) {
                    ...
                    if (empty($ret)) {
                        ...
                        foreach ($request->getUploadedFiles() as $item => $file) {
                            if (preg_match('/^file_(logo)$/', $item, $match)) {
